...
class AiHandler
{
    ...
    {
        if (empty($apiKey)) {
            throw new \Exception('Google API Key belum diisi.');
        }
    ...
    {
        if (empty($apiKey)) {
            throw new \Exception('OpenAI API Key belum diisi.');
        }
    ...
    private function askOpenAICompatible($provider, $profile, $systemPrompt, $message, $history = [])
    {
        ...
        if (empty($apiKey)) {
            throw new \Exception("API Key untuk {$provider} kosong.");
        }
    ...
    private function getDynamicKnowledge(): string
    {
        ...
        $knowledge .= "\nPENDIDIKAN:\n";
        foreach (array_slice($eduStats, 0, 8) as $label => $count) {
            $knowledge .= "- {$label}: {$count} orang\n";
        }

        $knowledge .= "\nPEKERJAAN UTAMA:\n";
        foreach (array_slice($jobStats, 0, 8) as $label => $count) {
            $knowledge .= "- {$label}: {$count} orang\n";
        }

        $knowledge .= "\nAGAMA:\n";
        foreach ($religionStats as $label => $count) {
            $knowledge .= "- {$label}: {$count} orang\n";
        }
